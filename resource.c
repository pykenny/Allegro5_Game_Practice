...
g_resr *create_gameplay_resr() {
    g_resr* gres_ptr = calloc(1, sizeof(g_resr));
    gameplay_resr *gpres_ptr = &(gres_ptr->gameplay_resr);
    gpres_ptr->sg = create_stage();
    return gres_ptr;
}

void destroy_gameplay_resr(g_resr *p) {
    if (p) {
        free_stage(p->gameplay_resr.sg);
        free(p);
    }
    return;
}